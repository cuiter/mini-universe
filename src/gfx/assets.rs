...
impl<'a> Assets<'a> {
    pub fn load(creator: &'a mut TextureCreator<WindowContext>) -> Assets<'a> {
        let assets_dir_pathbuf = PathBuf::from(get_executable_path().unwrap().parent().unwrap().parent().unwrap().parent().unwrap());

        let mut agent_sprite_pathbuf = assets_dir_pathbuf.clone();
        agent_sprite_pathbuf.push(AGENT_SPRITE_PATH);
        let agent_sprite = creator.load_texture(agent_sprite_pathbuf.as_path()).unwrap();

        let mut agent_eyes_sprite_pathbuf = assets_dir_pathbuf.clone();
        agent_eyes_sprite_pathbuf.push(AGENT_EYES_SPRITE_PATH);
        let agent_eyes_sprite = creator.load_texture(agent_eyes_sprite_pathbuf.as_path()).unwrap();
